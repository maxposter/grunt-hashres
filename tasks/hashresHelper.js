        ...
        options.files.forEach(function(f) {
            f.src.forEach(function(src) {
                var md5        = utils.md5(src).slice(0, 8),
                    ...
                    fullPath = (path.dirname(src) + '/' + path.basename(src)).replace(fileBasePath, ''),
                    renamed    = formatter({
                        hash: md5,
                    ...
                    nameSearch = searchFormatter({
                        hash: /[0-9a-f]{8}/,
                        name: fullPath.slice(0, fullPath.lastIndexOf('.')),
                        ext: fileName.slice(lastIndex + 1, fileName.length)
                    });

                // Mapping the original name with hashed one for later use.
                nameToHashedName[fullPath] = formatter({
                    hash: md5,
                    name: fullPath.slice(0, fullPath.lastIndexOf('.')).replace(/\.hash-[0-9a-f]{8}/g, ''),
                    ext : fileName.slice(lastIndex + 1, fileName.length)
                });
                nameToNameSearch[fullPath] = nameSearch;
            ...
            if (options.mapFile) {
                ...
                fs.writeFileSync(options.mapFilePath + 'assets.json', JSON.stringify(mappingFiles, null, 4));
            } else {
